package com.meitu.library.example.gl.util;
...
import com.meitu.library.example.TestApplication;
...
public class MTGLUtil {
    ...
    public static String readAssetsText(String filePath) {
        try {
            InputStream is = TestApplication.getApplication().getAssets().open(filePath);
    ...
    public static void saveTexture(Bitmap bitmap, int index) {
        File folder = new File(cacheTexturePath);
        if (!folder.exists() && !folder.mkdirs()) {
            MTGLUtil.d(TAG, "path is not exist");
            return;
        }
        final String jpegName = cacheTexturePath + "des" + index + ".jpg";
        try {
