...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create_base(const char *fragment_shader_source) {
    ...
    renderer->vertex_shader = IJK_GLES2_loadShader(GL_VERTEX_SHADER,
    ...
    IJK_GLES2_checkError("glCreateProgram");
    if (!renderer->program)
        goto fail;

    glAttachShader(renderer->program, renderer->vertex_shader);
    IJK_GLES2_checkError("glAttachShader(vertex)");
    ...
    IJK_GLES2_checkError_TRACE("glGetAttribLocation(av4_Position)");
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create(SDL_VoutOverlay *overlay) {
    ...
    switch (overlay->format) {
        case SDL_FCC_RV16:
            renderer = IJK_GLES2_Renderer_create_rgb565();
            break;
        case SDL_FCC_RV24:
            renderer = IJK_GLES2_Renderer_create_rgb888();
            break;
        case SDL_FCC_RV32:
...
#endif
        case SDL_FCC_YV12:
            renderer = IJK_GLES2_Renderer_create_yuv420p(overlay->filter);
            break;
        case SDL_FCC_I420:
            renderer = IJK_GLES2_Renderer_create_yuv420p(overlay->filter);
            break;
        case SDL_FCC_I444P10LE:
            renderer = IJK_GLES2_Renderer_create_yuv444p10le();
            break;
        default:
...
GLboolean IJK_GLES2_Renderer_setupGLES() {
    ...
    IJK_GLES2_checkError_TRACE("glClearColor");
    glEnable(GL_CULL_FACE);
    IJK_GLES2_checkError_TRACE("glEnable(GL_CULL_FACE)");
    glCullFace(GL_BACK);
    IJK_GLES2_checkError_TRACE("glCullFace");
...
GLboolean
IJK_GLES2_Renderer_setGravity(IJK_GLES2_Renderer *renderer, int gravity, GLsizei layer_width,
                              GLsizei layer_height) {
    if (renderer->gravity != gravity && gravity >= IJK_GLES2_GRAVITY_MIN &&
        gravity <= IJK_GLES2_GRAVITY_MAX)
...
static void
IJK_GLES2_Renderer_TexCoords_cropRight(IJK_GLES2_Renderer *renderer, GLfloat cropRight) {
...
GLboolean IJK_GLES2_Renderer_renderOverlay(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay) {
    ...
    IJK_GLES2_checkError_TRACE("glClear");
