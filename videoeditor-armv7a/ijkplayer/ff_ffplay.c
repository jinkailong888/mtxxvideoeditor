...
char *filter_video_movie = "movie='/storage/emulated/0/VideoEditorDir/save.png',scale=50:50[wm];[in][wm]overlay=W-w-5:H-h-5[out]";
...
char *filter_audio_descr = "[in0][in1]amix=inputs=2[out]";
char *audio_name1= "in0";
char *audio_name2= "in1";
#else
char *filter_audio_descr = NULL;
char *audio_name1 = NULL;
char *audio_name2 = NULL;
#endif
...
/**
 * 从 AVPacket 队列中取出一个 AVPacket
 * @param q  AVPacket 队列
 * @param pkt [out] 取出的AVPacket
 * @param block
 * @param serial
 * @return  如果播放暂停返回-1;如果成功取出返回1; 0 if no packet and > 0 if packet.
 */
...
static int
convert_image(FFPlayer *ffp, AVFrame *src_frame, int64_t src_frame_pts, int width, int height) {
    ...
    if (ret >= 0 && got_packet > 0) {
        ...
        if (img_info->count <= 0)
            ffp_notify_msg4(ffp, FFP_MSG_GET_IMG_STATE, (int) src_frame_pts, 1, file_name,
                            file_name_length);
        else
            ffp_notify_msg4(ffp, FFP_MSG_GET_IMG_STATE, (int) src_frame_pts, 0, file_name,
                            file_name_length);
    ...
    do {
        ...
        if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
            ...
            do {
                ...
                if (packet_queue_get_or_buffering(ffp,
                                                  d->queue,
                                                  &pkt,
                                                  &d->pkt_serial,
                                                  &d->finished) < 0)
...
static size_t parse_ass_subtitle(const char *ass, char *output) {
    char *tok = NULL;
    tok = strchr(ass, ':');
    if (tok) tok += 1; // skip event
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip layer
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip start_time
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip end_time
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip style
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip name
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip margin_l
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip margin_r
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip margin_v
    tok = strchr(tok, ',');
    if (tok) tok += 1; // skip effect
    if (tok) {
        char *text = tok;
        size_t idx = 0;
        do {
...
static double compute_target_delay(FFPlayer *ffp, double delay, VideoState *is) {
    ...
    //如果视频时钟不是主时钟,则通过 重复显示 或 删除 视频帧来调整显示的正确性
    if (get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER) {

        //视频时钟与主时钟（默认为音频时钟）的差值
        ...
        //限制在 AV_SYNC_THRESHOLD_MIN 0.04 和 AV_SYNC_THRESHOLD_MAX 0.1 之间，如果超出范围则返回阈值
        sync_threshold = FFMAX(AV_SYNC_THRESHOLD_MIN, FFMIN(AV_SYNC_THRESHOLD_MAX, delay));


        /* -- by bbcallen: replace is->max_frame_duration with AV_NOSYNC_THRESHOLD */
        if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD) {
            if (diff <= -sync_threshold)//如果视频时钟减主时钟差值
...
static void video_refresh(FFPlayer *opaque, double *remaining_time) {
    ...
    if (!is->paused
        &&
        get_master_sync_type(is) == AV_SYNC_EXTERNAL_CLOCK  //音视频同步类型
        &&
        is->realtime)
...
//            return 0;
        ...
        if (!isnan(pts)) {
            ...
            if ((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                ...
                if (is->drop_vframe_count == 0) {
                    ...
                    av_log(NULL, AV_LOG_INFO,
                           "video accurate_seek start, is->seek_pos=%lld, pts=%lf, is->accurate_seek_time = %lld\n",
                ...
                if ((now - is->accurate_seek_start_time) <= ffp->accurate_seek_timeout) {
                    ...
                    av_log(NULL, AV_LOG_WARNING,
                           "video accurate_seek is error, is->drop_vframe_count=%d, now = %lld, pts = %lf\n",
    ...
    //获取帧缓冲队列的可写入位置，若队列满了，则等待至队列中有空闲的位置
...
int configure_video_filtergraph(AVFilterGraph *graph, const char *filtergraph,
    ...
    av_log(NULL, AV_LOG_DEBUG, "configure_video_filtergraph   filtergraph:%s", filtergraph);
    if (filtergraph) {
...
int configure_audio_filtergraph(AVFilterGraph *graph, const char *filtergraph,
                                AVFilterContext *source_ctx1, AVFilterContext *source_ctx2,
                                AVFilterContext *sink_ctx) {
    int ret, i;
    int nb_filters = graph->nb_filters;
    AVFilterInOut *outputs1 = NULL, *outputs2 = NULL, *inputs = NULL;
    av_log(NULL, AV_LOG_DEBUG, "configure_audio_filtergraph   filtergraph:%s", filtergraph);
    AVFilterInOut *filter_outputs[2];
    if (filtergraph) {
        outputs1 = avfilter_inout_alloc();
        outputs2 = avfilter_inout_alloc();
        inputs = avfilter_inout_alloc();
        if (!outputs1 || !outputs2 || !inputs) {
            ret = AVERROR(ENOMEM);
            goto fail;
        }

        outputs1->name = av_strdup(audio_name1);
        outputs1->filter_ctx = source_ctx1;
        outputs1->pad_idx = 0;
        outputs1->next = NULL;

        outputs2->name = av_strdup(audio_name2);
        outputs2->filter_ctx = source_ctx2;
        outputs2->pad_idx = 0;
        outputs2->next = NULL;
        ...
        if ((ret = avfilter_graph_parse_ptr(graph, filtergraph, &inputs, filter_outputs, NULL)) < 0)
            goto fail;
    } else {
        av_log(NULL, AV_LOG_ERROR, "configure_audio_filtergraph  !filtergraph");
    }
...
int
configure_video_filters(FFPlayer *ffp, AVFilterGraph *graph, VideoState *is, const char *vfilters,
                        AVFrame *frame) {
...
//    }
    ...
    if (fabsf(ffp->pf_playback_rate) > 0.00001 &&
        ...
        av_log(ffp, AV_LOG_INFO, "vf_rate=%f(1/%f)\n", ffp-f>pf_playback_rate, rate);
        snprintf(setpts_buf, sizeof(setpts_buf), "%f*PTS", rate);
        INSERT_FILT("setpts", setpts_buf);
    }
#endif

    av_log(NULL, AV_LOG_DEBUG, "configure_video_filters configure_filtergraph   vfilters:%s",
           vfilters);

    if ((ret = configure_video_filtergraph(graph, vfilters, filt_src, last_filter)) < 0)
        goto fail;
...
static int configure_audio_filters(FFPlayer *ffp, const char *afilters, int force_output_format) {
    ...
    int channels[2] = {0, -1};
    AVFilterContext *filt_asrc1 = NULL, *filt_asrc2 = NULL, *filt_asink = NULL;
    ...
    ret = snprintf(asrc_args1, sizeof(asrc_args1),
                   "sample_rate=%d:sample_fmt=%s:channels=%d:time_base=%d/%d",
                   is->audio_filter_src.freq, av_get_sample_fmt_name(is->audio_filter_src.fmt),
                   is->audio_filter_src.channels,
                   1, is->audio_filter_src.freq);
    if (is->audio_filter_src.channel_layout)
        snprintf(asrc_args1 + ret, sizeof(asrc_args1) - ret,
                 ":channel_layout=0x%"PRIx64, is->audio_filter_src.channel_layout);


    ret = snprintf(asrc_args2, sizeof(asrc_args2),
                   "sample_rate=%d:sample_fmt=%s:channels=%d:time_base=%d/%d",
                   is->audio_filter_src.freq, av_get_sample_fmt_name(is->audio_filter_src.fmt),
                   is->audio_filter_src.channels,
                   1, is->audio_filter_src.freq);
    if (is->audio_filter_src.channel_layout)
        snprintf(asrc_args2 + ret, sizeof(asrc_args2) - ret,
                 ":channel_layout=0x%"PRIx64, is->audio_filter_src.channel_layout);


    ret = avfilter_graph_create_filter(&filt_asrc1,
                                       avfilter_get_by_name("abuffer"), audio_name1,
                                       asrc_args1, NULL, is->agraph);
    if (ret < 0)
        goto end;

    ret = avfilter_graph_create_filter(&filt_asrc2,
                                       avfilter_get_by_name("abuffer"), audio_name2,
                                       asrc_args2, NULL, is->agraph);
    if (ret < 0)
        goto end;
...
#endif
    if ((ret = configure_audio_filtergraph(is->agraph, afilters_args[0] ? afilters_args : NULL,
                                           filt_asrc1, filt_asrc2,
                                           filt_asink)) < 0)
        goto end;

    is->in_audio_filter1 = filt_asrc1;
    is->in_audio_filter2 = filt_asrc2;
        ...
        if (got_frame) {
            ...
            if (ffp->enable_accurate_seek && is->audio_accurate_seek_req && !is->seek_req) {
                ...
                if (!isnan(frame_pts)) {
                    ...
                    if ((audio_clock * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                        ...
                        if (audio_seek_pos == is->seek_pos) {
                            av_log(NULL, AV_LOG_INFO,
                                   "audio accurate_seek is ok, is->drop_aframe_count=%d, audio_clock = %lf\n",
                                   is->drop_aframe_count, audio_clock);
                            is->drop_aframe_count = 0;
                            SDL_LockMutex(is->accurate_seek_mutex);
                            is->audio_accurate_seek_req = 0;
                            SDL_CondSignal(is->video_accurate_seek_cond);
                            if (audio_seek_pos == is->seek_pos && is->video_accurate_seek_req &&
                                !is->abort_request) {
                ...
                if (audio_accurate_seek_fail) {
                    av_log(NULL, AV_LOG_INFO,
                           "audio accurate_seek is error, is->drop_aframe_count=%d, now = %lld, audio_clock = %lf\n",
...
//        print_avframe_tag(frame, "get_video_frame");
        ...
        if (ffp->get_frame_mode) {
            av_log(NULL, AV_LOG_DEBUG, "ffplay_video_thread get_frame_mode");
            ...
            if (dst_pts < 0) {
                ...
                dst_pts += (ffp->get_img_info->end_time - ffp->get_img_info->start_time) /
                           (ffp->get_img_info->num - 1);
            }

            pts = (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(tb);
            pts = pts * 1000;
            av_log(NULL, AV_LOG_ERROR, "get_frame_mode pts = %lf\n", pts);
            if (pts >= dst_pts) {
...
//        av_log(NULL, AV_LOG_DEBUG, "frame->format %s ",
//               SDL_AMediaCodec_getColorFormatName(frame->format));
//        av_log(NULL, AV_LOG_DEBUG, "frame->format %d, planes=%d, frame->data[0] %d ", frame->format,
//               planes, frame->data[0]);
        ...
        if (last_w != frame->width
            ...
            char *filter = filter_video_descr;
            av_log(NULL, AV_LOG_DEBUG, "set video filter:%s", filter);
...
static int video_thread(void *arg) {
    ...
    if (ffp->node_vdec) {
        //根据软硬解码设置的不同调用不同的方法
        //软解调用 ffp_video_thread 方法
        //硬解调用 ffpipenode_android_mediacodec_vdec.c 的 func_run_sync 方法
...
//    }
    ...
    if (is->swr_ctx) {
        ...
        if (ffp->soundtouch_enable && ffp->pf_playback_rate != 1.0f && !is->abort_request) {
            ...
            int ret_len = ijk_soundtouch_translate(is->handle, is->audio_new_buf,
                                                   (float) (ffp->pf_playback_rate),
                                                   (float) (1.0f / ffp->pf_playback_rate),
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len) {
    ...
    if (!isnan(is->audio_clock)) {
        set_clock_at(&is->audclk, is->audio_clock - (double) (is->audio_write_buf_size) /
                                                    is->audio_tgt.bytes_per_sec -
                                  SDL_AoutGetLatencySeconds(ffp->aout), is->audio_clock_serial,
...
static int audio_open(FFPlayer *opaque, int64_t wanted_channel_layout, int wanted_nb_channels,
    ...
    wanted_spec.samples = FFMAX(SDL_AUDIO_MIN_BUFFER_SIZE, 2
            << av_log2(wanted_spec.freq / SDL_AoutGetAudioPerSecondCallBacks(ffp->aout)));
    ...
    SDL_AoutSetDefaultLatencySeconds(ffp->aout,
                                     ((double) (2 * spec.size)) / audio_hw_params->bytes_per_sec);
    return spec.size;
}
...
static int stream_component_open(FFPlayer *ffp, int stream_index) {
    ...
    //设置打开解码器时的配置项
    opts = filter_codec_opts(ffp->codec_opts,
                             avctx->codec_id,
                             ic,
...
static int
stream_has_enough_packets(AVStream *st, int stream_id, PacketQueue *queue, int min_frames) {
    return stream_id < 0 ||
           queue->abort_request ||
...
/**
 * 文件读取线程，主要用于读取AVPacket：
 * 1.打开文件，分离音视频流
 * 2.若存在音/视频流，则创建对应的解码线程
 * 3.设置 metadata 信息
 * 4.读取音/视频的 AVPacket ，加到  AVPacket 缓冲队列
 * 5.是否读取受限于播放状态、AVPacket 缓冲队列大小
 *
 * @param arg FFPlayer
 * @return
 */
static int read_thread(void *arg) {
    ...
    if (is->video_stream < 0 && is->audio_stream < 0) {
        av_log(NULL, AV_LOG_FATAL,
               "Failed to open file '%s'，is->video_stream < 0 && is->audio_stream < 0 \n",
               is->filename);
        ret = -1;
        goto fail;
    }
    ...
    if (is->video_st && is->video_st->codecpar) {
        AVCodecParameters *codecpar = is->video_st->codecpar;
        //通知上层视频宽高信息，对于旋转的有角度的视频，此处通知的宽高有误
...
//      of the seek_pos/seek_rel variables
        ...
        //如果播放完
        if ((!is->paused || completed)//如果未暂停 或 播放完毕
            &&
            //如果没有音频流或音频播放完
            ...
            //loop设为0则无限循环播放，设为1代表只播一次
            if (ffp->loop != 1 && (!ffp->loop || --ffp->loop)) {//如果循环播放
...
//            print_avpacket_tag(pkt, "播放器 刚读出来的视频包");
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat) {
    ...
    // 创建音视频解码后的数据（AVFrame）存放队列, 第三个参数为队列可容放最大值
    ...
    // 创建数据读取线程，其内部创建了解码线程
    is->read_tid = SDL_CreateThreadEx(&is->_read_tid,
                                      read_thread,
                                      ffp,
                                      "ff_read");
...
void ffp_set_frame_at_time(FFPlayer *ffp, const char *path, int64_t start_time, int64_t end_time,
...
int ffp_packet_queue_get_or_buffering(FFPlayer *ffp, PacketQueue *q, AVPacket *pkt, int *serial,
                                      int *finished) {
...
int ffp_queue_picture(FFPlayer *ffp, AVFrame *src_frame, double pts, double duration, int64_t pos,
                      int serial) {
